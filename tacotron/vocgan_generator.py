...
class Generator(nn.Module):
    ...
    def generate_audio(self, predicted_postnet_mel_specs, **kwargs):
        if len(predicted_postnet_mel_specs.shape) == 2:
            predicted_postnet_mel_specs = predicted_postnet_mel_specs.unsqueeze(0)

        audio = self.infer(predicted_postnet_mel_specs).squeeze()
